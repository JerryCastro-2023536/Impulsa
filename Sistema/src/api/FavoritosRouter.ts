import { IncomingMessage, ServerResponse } from "http";
import { FavoritosService } from "../service/FavoritosService";
import { sendJson } from "./SendJson";
import { ReadBody } from "./readBody";
import { FavoritosValidate } from "../validators/FavoritosValidate";

const fs = new FavoritosService();

export async function FavoritoRouter(req: IncomingMessage, res : ServerResponse) : Promise<boolean>{
    const url = req.url ?? "";
    const [path] = url.split("?");
    const segmentos = path.split("/").filter(Boolean);
    if(segmentos.length === 1 && segmentos[0] === "favoritos"){
        if(req.method === "GET"){
            return sendJson(res, 200, await fs.mostrarFavoritos()), true;
        }

        if(req.method === "POST"){
            const body = await ReadBody(req);
            const fav = JSON.parse(body);
            const errores = await FavoritosValidate(fav);
            if (errores.length > 0) {
                return sendJson(res, 400, { errores }), true;
            }
            const resultado = await fs.agregarFavorito(fav);
            return sendJson(res, 201, resultado), true;
        }
        return sendJson(res, 405, { error: "Método no permitido" }), true;
    }

    if(segmentos.length === 2 && segmentos[0] === "favoritos"){
        const id = Number(segmentos[1]);

        if(req.method === "GET"){
            const fav = await fs.buscarFavoritoPorId(id);
            return sendJson(res, 200, fav), true;
        }

        if(req.method === "PUT"){
            try{
                const body = await ReadBody(req);
                const fav = JSON.parse(body);
                const errores = await FavoritosValidate(fav);
                if (errores.length > 0) {
                    return sendJson(res, 400, { errores }), true;
                }
                const resultado = await fs.actualizarFavorito(id, fav);
                return sendJson(res, 200, resultado), true;
            }catch(error){
                return sendJson(res, 400, { error: error instanceof Error ? error.message : "JSON inválido o campos incompletos." }), true;
            }
        }

        if(req.method === "DELETE"){
            const resultado = await fs.eliminarFavorito(id);
            return sendJson(res, resultado ? 200 : 404, resultado ? { message: "Favorito eliminado." } : { error: "Favorito no encontrado." }), true;
        }
        return sendJson(res, 405, { error: "Método no permitido" }), true;
    }
    return false;
}